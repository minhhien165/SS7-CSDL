...
select 
employees.name,
workreports.report_content
from employees
join workreports on employees.employee_id = workreports.employee_id;
select
employees.name,
...
update employees
set employees.salary = 6500.00
where name like '%Alice%';
delete workreports 
from workreports 
join employees on workreports.employee_id = employees.employee_id 
where employees.name like "%Brown%";
insert into employees (name, date_of_birth, department_id, salary) 
